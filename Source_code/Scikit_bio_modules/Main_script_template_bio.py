...
File_path = "" # Put the path to the Excel file with data here.
...
Environment_vars = [""] #List of names from the environment sheet to use for the analysis.
Drop_row = [] #List of names of the observation points to disregard for the analysis. (Will be removed from both the Species and Environment), can be left empty.
...
LogScale_Environment = False # Same as the line above, but for the environmental variables.
...
Subfigure = False # Is False by default. Can set to a string containing a letter to include it in the top left corner of the figure.
