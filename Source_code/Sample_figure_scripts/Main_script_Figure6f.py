...
File_path = "../../Sample_data/Restructured_input_data.xlsx" # Or put own path to Restructured_input_data.xlsx here
...
Set_Null = "average" #Ordination methods require all cells to be filled. Set to False to remove rows with missing values. Set to "average" to replace the missing values with the average of their corresponding variable. Set to "median" to replace the missing values with the median of their corresponding variable. Set to any numeric value to replace all empty cells with that value.

# Transform parameters
Center = False #When set to true, centers the data; giving it an average of 0 
Standardize = False #When set to true, centers data and divides by standard deviation
LogScale_Species = ["Benzene carboxylase", "NarG", "Peptococcus", "NirS", "BssA_SRB", "Total bacteria 16SRrna", "BssA nitraat"] #Set to false to disable log transformation, set to True when wanting to log transform all data. Otherwise provide a list of all variable column names you want to transform
LogScale_Environment = ["N", "pm-X", "Sum GC", "Indene", "T", "B", "E"] # Same as the line above, but for environmental variables.
...
Subfigure = "f" # Is False by default. Can set to a string containing a letter to include it in the top left corner of the figure.
...
Arrow_width = 0.003 # Width of the vector tail 
Arrowhead_width = 0.03 # Width of the vector head
Arrowhead_length = 0.03 # length of the vector head
