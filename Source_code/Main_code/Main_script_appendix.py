...
if not Species_sheet and not Environment_sheet:
    raise Exception("Both the Species_sheet and Environment_sheet parameter are empty. Please provide at least one one sheet in one of the parameters.")
...
if Species_sheet:
    Species = ana.reader(InputFile = InputFile, Units = Units, Select_sheet = Species_sheet, Row_name = Row_name, Drop_variable = Drop_species, verbose = verbose)
...
if Environment_sheet:
    Environment = ana.reader(InputFile = InputFile, Units = Units, Select_sheet = Environment_sheet, Row_name = Row_name, Drop_variable = Drop_Environment, verbose = verbose)
...
elif Method == "RDA" and subMed == "eco":
    results, names = ana.RDAeco(data = data, verbose = verbose)

elif Method == "RDA" and subMed == "sci":
